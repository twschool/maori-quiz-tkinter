...
This module is all the previous modules combined into one
...
import random
import pygame
from vehicles_vars import NPCVehicle, SpecialVehicle, TruckVehicle, \
    EnemyVehicle, EnemyLanes, PLAYER_PREFIX, IMAGE_SUFFIX
...
class Coordinate:
    """A class to represent the x, y coordinates of the player and the enemy vehicles."""
    ...
    def format(self):
        """Return the players formatted coordinates (x, y) as a tuple"""
...
class Sprite(Coordinate):
    ...
    def __init__(self, x, y, image_object: pygame.Surface, scale=1.0):
...
class Background:
    ...
    def __init__(self):
        ...
        # Start the second image immediately above the first
        self.y2 = -background_image_object.get_height()
...
class Enemy(Sprite):
    ...
    def move(self):
        ...
        # Random speed
        self.y += global_enemy_velocity + random.uniform(0, 0.5)
...
class GameController:
    ...
    def update(self, screen: pygame.Surface, player_object: Player,
        ...
        # Change the enemy spawn cooldown if it hasn't been reset
        spawn_cooldown -= 1
...
def make_background_object() -> pygame.Surface:
    ...
    new_background_image = pygame.transform.scale(background_image,
                                                  (int(background_image.get_width() *
                                                       background_scale_x),
...
def display_text(screen: pygame.Surface, text: str,
                 font_color=(255, 255, 255), x=DISPLAY_SIZE[0] // 2, y=0 + 100,
...
def start_screen():
    ...
    display_text(screen, "Untitled Racing Game – By Thomas",
                 background=BACKGROUND_GRAY, font_color=ORANGE, y=20)
    display_text(screen, "Instructions:",
                 background=BACKGROUND_GRAY, font_color=ORANGE, y=70)
    display_text(screen, "Move left and right with the arrow keys and avoid",
                 background=BACKGROUND_GRAY, font_color=ORANGE, y=100)
    display_text(screen, "hitting other cars. How far can you get?",
                 background=BACKGROUND_GRAY, font_color=ORANGE, y=130)
...
def game_over_text():
    display_text(screen, "Game Over", y=20, font_color=NICE_RED,
                 background=BACKGROUND_GRAY, is_big=True)

    if score > highscore:
        display_text(screen, f"New Highscore!", y=500, font_color=GREEN,
                     background=BACKGROUND_GRAY, is_big=True)

    # Display the players score
    display_text(screen, f"Score: {score}", y=150, font_color=ORANGE,
                 background=BACKGROUND_GRAY, is_big=True)

    # Display the highscore
    display_text(screen, f"Highscore: {highscore}", y=200, font_color=ORANGE,
                 background=BACKGROUND_GRAY, is_big=True)
...
def death_menu():
    ...
    # orange = ORANGE
    game_over_text()
    display_text(screen, "Press [Enter] to play again or", y=500, font_color=ORANGE,
                 background=BACKGROUND_GRAY)
    display_text(screen, "Press [Esc] to exit", y=520, font_color=ORANGE,
                 background=BACKGROUND_GRAY)
    pygame.display.update()
    while in_menu:
        for event in pygame.event.get():
...
Defines the y upper and lower bound to give the illusion of screen wrapping for the background
  The upper bound is the highest the y of the image can be
  so that the full image is displayed on the screen
  The lower bound is the lowest the image can be so the full image is displayed on the screen
  When the y is 0 the image isn't displayed on the screen (fully offscreen)"""
...
while not finished:
    ...
    display_text(screen, f"Score: {score}",
                 x=0, y=0, font_color=(120, 120, 255),
                 background=BACKGROUND_GRAY, center=False)

    score_display = highscore if score < highscore else score

    display_text(screen, f"Highscore: {score_display}",
                 x=0, y=20, font_color=(120, 120, 255),
                 background=BACKGROUND_GRAY, center=False)
