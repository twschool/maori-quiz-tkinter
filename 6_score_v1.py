"""
v1 of the score module
this module will add one to the score when a car is passes and despawns
"""
...
while not finished:
    ...
    if has_colided:
        ...
        animation_fps = 10
        display_text(screen, "Game Over", x=DISPLAY_SIZE[0] // 2, y=DISPLAY_SIZE[1] // 2)
        ...
        end_text = f"Score: {score} - Highscore: {highscore}"

        if score > highscore:
            with open(HIGHSCORE_FILENAME, "w") as file:
                file.write(str(score))
                
            end_text = f"NEW HIGHSCORE! - Highscore: {score}"
